...
triqs::h5::file out_file("atom_diag_real.h5", H5F_ACC_TRUNC);
#else
...
TEST(atom_diag_real, Autopartition) {
 ...
 for(int sp : range(ad.n_subspaces())) {
  ...
  EXPECT_ARRAY_NEAR(make_unit_matrix<double>(sp_dim[sp]),
  ...
  EXPECT_ARRAY_NEAR(umat_ref[sp] * e_diag * transpose(umat_ref[sp]),
                    umat[sp] * e_diag * transpose(umat[sp]));
 ...
 std::vector<std::vector<matrix<double>>>
  c_matrices(fops.size(), std::vector<matrix<double>>(ad.n_subspaces()));
 std::vector<std::vector<matrix<double>>>
  cdag_matrices(fops.size(), std::vector<matrix<double>>(ad.n_subspaces()));
